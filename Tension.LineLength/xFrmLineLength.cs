...
namespace Tension.LineLength
{
    ...
    public partial class xFrmLineLength : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void xFrmLineLength_Load(object sender, EventArgs e)
        {
            IniDevice();
        }

        private void xFrmLineLength_FormClosing(object sender, FormClosingEventArgs e)
        {
            ExitApp();
        }
        ...
        private void IniDevice()
        {
            ...
            Text = string.Format("{0}线长检测实时采集系统",_lineName);
        }
        ...
        private void PlcScan(object idx)
        {
            ...
            while (true)
            {
               ...
               //         LogHelper.WriteFile(string.Format("Device:{0},TurnNoPre:{1},TurnNoAct:{2}", i, _ltTurnNoPre[i], _ltTurnNoAct[i]));
                    ...
                    Invoke(new Action(() =>
                    {
                        gdLineLen.DataSource = _ltTurnCnt;
                        gdLineLen.RefreshDataSource();
                    }));
                }
                catch (Exception ex)
                {
